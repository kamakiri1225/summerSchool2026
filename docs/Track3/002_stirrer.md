...
メッシュは中央部だけ意図的にV字に傾けてある。流れ方向とセルの向きがずれる部分を作ることで、後述の**偽拡散**（斜めメッシュでのなまり）が現れやすくしている。上図でも、傾いた領域に入ったところから界面がぼやけ始めているのが分かる。
...
つまり、なまりの主因はスキームの数値拡散ではなく、**格子が流れに斜交することで生じる偽拡散（false diffusion）**である。
