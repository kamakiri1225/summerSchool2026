...
![題材の形状とブロック分割（中央のダイヤ型で流れに対して斜めの格子ができる）](img/000_ex/rect_diamond_black.svg)
...
- 計算は2次元なので、後述の「斜めメッシュでのなまり」は $x$・$y$ 方向が絡む**数値拡散**であり、1次元の式だけでは見えない。

![移流問題のセットアップ（上30℃・下50℃を右へ流す。形を保ったまま伝搬するはず…）](img/002_stirrer/pdf_p023.svg)

今回は上図のような2次元領域で、**上側入口を30℃・下側入口を50℃**として右へ流す（拡散なしの純移流、`DT=0`）。理想的には上下の温度差（`y=25`の界面）がシャープなまま出口まで運ばれるはずである。

メッシュは中央部だけ意図的にV字に傾けてある。流れ方向とセルの向きがずれる部分を作ることで、**格子が流れに対して斜めであることによる数値拡散**が現れやすいようにしている。上図でも、傾いた領域に入ったところから界面がぼやけ始めているのが分かる。
...
上の4枚をよく見ると、スキームによらず共通の傾向がある。格子が流れに整列している左半分では界面がシャープなのに対し、V字に傾いた右半分ではどれもなまっている。

つまり、なまりの主因はスキームの選び方そのものではなく、**格子が流れに対して斜めになっていることで生じる数値拡散**である。

実際、同じ `linearUpwind` でも、格子を流れに整列させた `blockMesh` で解けば、界面はシャープなまま左から右へ流れていく（下図）。精度はスキームの選択だけでなく、**格子を流れに沿わせられるかどうか**でも大きく変わる。
...
- **メッシュが流れに沿っているか**（流れに対して斜めだと数値拡散が出る）
