...
固定した面（`rotorPin`）は、下図のマゼンタで示した rotor1 / rotor2 セルゾーンの**外周表面全体**である。具体的には、

- **外周の円筒壁（半径 r=35mm、高さ z=10〜20mm と z=30〜40mm）** … これを固定することで rotor が半径方向へ広がらず、円形を保つ。
- **上下のフタ（z=10 / 20 / 30 / 40mm の水平な扇形面）** … これを固定することで rotor が軸方向へも動かない。


---

![固定した rotor 境界面 rotorPin（マゼンタ：外周円筒＋上下フタ）](img/002_stirrer/roter_pi.png)
